...
function AdvancedFilterByField({
    ...
    const selectedColor = alpha(theme.palette.text.secondary, 0.1)
    const notColor = alpha(theme.palette.error.main, 0.3)

    const selectStyle = context !== 'drawer'
        ? {backgroundColor: currentValue?.length > 0 ? selectedColor : undefined}
        : matchType === 'Is Not'
            ? {backgroundColor: currentValue.length > 0 ? notColor : undefined}
            : {backgroundColor: currentValue.length > 0 ? selectedColor : undefined}

    const {isMobile} = useWindowSize()
    const marginBottom = context === 'drawer' ? 18 : 12
    const fieldWidth = isMobile || context === 'drawer' ? 210 : 300
